          ...
          <nav className="w-full px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
            <div className="flex items-center justify-between">
              <a className="flex-none text-xl font-semibold dark:text-white
               text-white md:px-20  focus:ring- hidden sm:block" href="#" aria-label="Brand"> Fonotheapp
              </a>
              <a className="flex-none text-xl font-semibold dark:text-white
               text-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-600 sm:hidden" href="#" aria-label="Brand">Fonotheapp</a>
            ...
            <div id="navbar-collapse-with-animation" className={`hs-collapse ${!showMenu && "hidden"}
             md:px-20 overflow-hidden transition-all duration-300 basis-full grow sm:block`}>
              <div className="flex flex-col gap-5 mt-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
                <Link to={"/"} onClick={() => setCurrentPage(1)} className={`font-medium ${currentPage === 1 ?
                "text-blue-500 hover:text-blu-400 " : "text-white hover:text-blu-400"}
                sm:py-3 sm:text-lg hover:text-blu-400 dark:text-blue-500 dark:focus:outline-none dark:focus:ring-1
              dark:focus:ring-gray-600 cursor-pointer `} aria-current="page">Início</Link>
                <Link to={"/como-funciona"} onClick={() => setCurrentPage(2)} className={`font-medium ${currentPage === 2 ?
                  "text-blue-500 hover:text-blu-400 " : "text-white hover:text-blu-400"}
                 sm:py-3 sm:text-lg hover:text-blu-400 dark:text-blue-500 dark:focus:outline-none dark:focus:ring-1
                  dark:focus:ring-gray-600 cursor-pointer `}>
                  Como funciona
                  </Link>
                <Link to={"/precos"} onClick={() => setCurrentPage(3)} className={`font-medium ${currentPage === 3 ?
                  "text-blue-500 hover:text-blu-400 " : "text-white hover:text-blu-400"}
                 sm:py-3 sm:text-lg hover:text-blu-400 dark:text-blue-500 dark:focus:outline-none dark:focus:ring-1
                  dark:focus:ring-gray-600 cursor-pointer `}>
                  Preços
                </Link>
                <Link to={"/login"} onClick={() => setCurrentPage(4)} className={`font-medium ${currentPage === 4 ?
                  "text-blue-500 hover:text-blu-400 " : "text-white hover:text-blu-400"}
                 sm:py-3 sm:text-lg hover:text-blu-400 dark:text-blue-500 dark:focus:outline-none dark:focus:ring-1
                  dark:focus:ring-gray-600 cursor-pointer `}>
                  Entrar
